...
wrong_answers = ["You speak to me like I can understand you.", "Consider that your assumptions are wrong: we don't speak the same "
                ...
                "been built another.", "This interface was written in a language.\n\nLike any language, it can fail.", "There's nothing "
                "left now but patience.\n\nYou are a rock, I am a machine."]
...
class Sequence(object):
    ...
    def game_play(self):
        ...
        while answer not in str(range_check) or answer.isdigit() == False:
            stdscr.clear()
            try:
                stdscr.addstr(1, 0, wrong_answers[wrong])
                wrong += 1
            except IndexError:
                stdscr.addstr(1,0, str(patience))
                patience += 1
            stdscr.refresh()
            stdscr.getch()
            stdscr.clear()
